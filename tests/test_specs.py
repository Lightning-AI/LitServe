...
@pytest.mark.asyncio
async def test_openai_embedding_spec_with_pred_yield(openai_embedding_request_data):
    spec = OpenAIEmbeddingSpec()
    server = ls.LitServer(TestEmbedAPIWithYieldPredict(), spec=spec)
...
@pytest.mark.asyncio
async def test_openai_embedding_spec_with_encode_response_yield(openai_embedding_request_data):
    spec = OpenAIEmbeddingSpec()
